  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      {services.map((service, index) => {
        ...
        if (isCorp) {
          // Corporate card — dark featured card
          return (
            <motion.article
              key={service.id}
              ...
              className="group md:col-span-2 lg:col-span-1 relative rounded-2xl overflow-hidden flex flex-col p-8 cursor-default"
              style={{
                background: "linear-gradient(135deg, #020b13 0%, #0a1e35 50%, #0057A8 100%)",
              }}
            >
              {/* Dot texture */}
              <div
                ...
                style={{
                  backgroundImage:
                    "radial-gradient(circle at 1px 1px, rgba(0,179,164,0.18) 1px, transparent 0)",
              ...
              <div
                className="absolute -bottom-12 -right-12 w-48 h-48 rounded-full pointer-events-none opacity-0 group-hover:opacity-100 transition-opacity duration-500"
                style={{ background: "radial-gradient(circle, rgba(0,179,164,0.25) 0%, transparent 70%)" }}
              />
              ...
              <div className="relative flex flex-col h-full">
                ...
                <Link
                  href="/contact"
                  className="inline-flex items-center gap-2 text-sm font-semibold text-turquoise hover:text-white transition-colors duration-200 mt-auto"
                >
                  Get Started
        ...
        return (
          <motion.article
            ...
            transition={{ delay: index * 0.08, duration: 0.5 }}
            whileHover={{ y: -8, transition: { type: "spring", stiffness: 350, damping: 22 } }}
            className="group relative bg-white rounded-2xl border border-gray-100 shadow-sm hover:shadow-xl hover:shadow-turquoise/8 transition-shadow duration-300 p-8 flex flex-col overflow-hidden cursor-default"
          >
            ...
            <Link
              href="/contact"
              className="inline-flex items-center gap-2 text-sm font-semibold text-turquoise hover:text-[#009ea0] transition-colors duration-200 mt-auto"
            >
              Get Started
              <ArrowRight className="h-4 w-4 transition-transform duration-200 group-hover:translate-x-1" />
            </Link>
